...
if(isset($_REQUEST["start_blog"])){
  ...
  $con = Dbcon();
  $usql = "INSERT INTO `blogs` (`title`, `descr`, `category`, `topics`, `author`, `cover_img`)".
  ...
  if($result == 'success'){
    header('Location: blog_compose.php?blog_reference='.$EVID);
  }else if($result == 'fail'){
    header('Location: blog_startup.php');
  }
...
if(isset($_REQUEST["save_blog"]) || isset($_REQUEST["publish_blog"])){
  ...
  $con = Dbcon();
  $usql = "UPDATE `blogs` SET `content`='$confile_loc', `status`='$newStat' WHERE `id`=$blog";
  if(mysqli_query($con, $usql)or die(mysqli_error($con))){
      $result = 'success';
  }else{$result = 'fail';}
  mysqli_close($con);
  ...
  if($result == 'success'){
    header('Location: blog_compose.php?blog_reference='.$blog);
  }else if($result == 'fail'){
    header('Location: blog_compose.php?blog_reference='.$blog);
  }
...
if(isset($_REQUEST["upload_media"])){
  ...
  $con = Dbcon();
  $usql = "INSERT INTO `blog_media`(`mname`, `msource`, `blog`, `author`)".
  ...
  if($result == 'success'){
    header('Location: blog_media.php?blog_reference='.$blog_id);
  }else if($result == 'fail'){
    header('Location: blog_media.php?blog_reference='.$blog_id);
  }
